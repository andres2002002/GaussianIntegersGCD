...
from logic.gaussian_factorization import factor_gaussian, are_associates

class FactorizationScreen(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        # --- Título ---
        self.title_label = ttk.Label(self, text="Factorizacion en primos", background="#2e2e2e", font=("Arial", 14, "bold"))
        ...
        self.calculate_button = ttk.Button(self, text="Factorizar", command=self.process)
    ...
    def process(self):
        try:
            ...
            result = 1
            for f in factors:
                result *= f  # Multiplicación acumulada de los factores
            
            result_str = f"La factorización de {z} es: {factor_str}\n"
            result_str += f"Verificación:\n {factor_str} = {result}"
            assoc, unit_corr = are_associates(result, z)
            if result == z:
                result_str += "\nVerificación exitosa!"
            elif assoc:
                result_str += f"\nEl resultado difiere del original por una unidad: {unit_corr}"
                result_str += f"\n{z} = {unit_corr} * {factor_str}"
            else:
                result_str += f"\n¡Error en la verificación! Se esperaba {z}, se obtuvo {result}"
