...
from logic.gaussian_gcd import gaussian_gcd

class GcdScreen(ttk.Frame):
    def __init__(self, parent, controller):
        ...
        # --- Título ---
        self.title_label = ttk.Label(self, text="Calcular MCD", background="#2e2e2e", font=("Arial", 14, "bold"))
        ...
        ttk.Label(input_frame, text="w:", font=(font, size, wight)).grid(row=1, column=0, padx=5, pady=5)
        self.real2 = ttk.Entry(input_frame, width=10)
        self.real2.grid(row=1, column=1, padx=5, pady=5)
        ttk.Label(input_frame, text="+", font=(font, size, wight)).grid(row=1, column=2)
        self.imag2 = ttk.Entry(input_frame, width=10)
        self.imag2.grid(row=1, column=3, padx=5, pady=5)
        ttk.Label(input_frame, text="i", font=(font, size, wight)).grid(row=1, column=4)

        # Botón para calcular la suma
        self.calculate_button = ttk.Button(self, text="MCD", command=self.process)
        ...
        self.result_text = tk.Text(self, height=10, width=40, wrap="word")
    ...
    def process(self):
        try:
            ...
            real2 = self.get_value_or_zero(self.real2)
            imag1 = self.get_value_or_zero(self.imag1)
            imag2 = self.get_value_or_zero(self.imag2)
            
            z = complex(real1, imag1)
            w = complex(real2, imag2)
            result, steps = gaussian_gcd(z, w)
            result_real = round(result.real)
            result_imag = round(result.imag)
            result = f"{result_real} + {result_imag}i"
            result_str = ""
            for step in steps:
                result_str += f"\n{step}"
